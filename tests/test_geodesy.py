...
class TestKmPerDegree:
    def test_it_is_derived_from_the_earth_radius(self):
        ...
        assert pytest.approx(R_EARTH * math.pi / 180.0) == KM_PER_DEG_LAT
        assert pytest.approx(111.1949, abs=1e-4) == KM_PER_DEG_LAT
...
class TestOffsetRoundTrip:
    ...
    def test_metres_and_kilometres_agree(self):
        assert offset_latlon_m(*RX, 1500.0, -800.0) == pytest.approx(offset_latlon(*RX, 1.5, -0.8))
...
class TestBistaticDelay:
    ...
    def test_delay_is_the_differential_over_c(self):
        tgt = (34.95, -82.15)
        assert bistatic_delay_us(*TX, *RX, *tgt) == pytest.approx(bistatic_differential_km(*TX, *RX, *tgt) / C_KM_US)
...
class TestPointInBeam:
    CFG = dict(rx_lat=RX[0], rx_lon=RX[1], tx_lat=TX[0], tx_lon=TX[1], beam_width_deg=41.0, max_range_km=50.0)
    ...
    def test_no_azimuth_means_omnidirectional(self):
        for brg in (0.0, 90.0, 180.0, 270.0):
            tgt = offset_latlon(*RX, 20 * math.sin(math.radians(brg)), 20 * math.cos(math.radians(brg)))
    ...
    def test_the_bistatic_limit_beats_the_circle_away_from_the_tx(self):
        ...
        tgt = offset_latlon(*RX, 45 * math.sin(math.radians(away)), 45 * math.cos(math.radians(away)))
        cfg = dict(self.CFG, max_range_km=60.0)
        assert point_in_beam(*tgt, beam_azimuth_deg=None, **cfg)
        assert not point_in_beam(*tgt, beam_azimuth_deg=None, max_bistatic_range_km=60.0, **cfg)
    ...
    def test_toward_the_tx_the_ellipse_reaches_further_than_the_circle(self):
        ...
        tgt = offset_latlon(
            *RX, (reach - 1) * math.sin(math.radians(toward)), (reach - 1) * math.cos(math.radians(toward))
        )
        assert point_in_beam(
            *tgt, beam_azimuth_deg=None, max_bistatic_range_km=60.0, **dict(self.CFG, max_range_km=30.0)
        )
    ...
    def test_a_node_without_a_bistatic_limit_keeps_the_circle(self):
        ...
        assert point_in_beam(*tgt, beam_azimuth_deg=None, **dict(self.CFG, max_range_km=60.0))
