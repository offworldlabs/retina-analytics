"""Tests for the superseded detection-level association path.

These moved out of test_association.py with the code they exercise.  They are
NOT redundant with the track-level tests: association_bench.py --mode detection
is the A/B baseline the track-level ghost figures are quoted against, and these
are the only guard on it.  If the baseline broke silently, that comparison would
become meaningless with nothing failing.
"""

from retina_analytics.detection_association import DetectionAssociator


class TestDetectionAssociator:
    def _make_assoc(self):
        assoc = DetectionAssociator(grid_step_km=5.0)
    ...
    def test_format_candidates_merges_cross_zone_pairs(self):
        ...
        """
        from retina_analytics.detection_association import AssociationCandidate
        ...
        assoc = DetectionAssociator(grid_step_km=3.0)
    ...
    def test_ghost_filter_rejects_clutter_pairing(self):
        ...
        from retina_analytics.association import OverlapZone
        from retina_analytics.detection_association import find_associations
