...
class TestExplicitAimRegistration:
    ...
    def test_missing_azimuth_falls_back_to_broadside(self):
        ...
        assert abs(assoc.node_geometries["Y"].beam_azimuth_deg - _broadside(_RX_A)) < 1e-4
    ...
    def test_none_azimuth_falls_back_to_broadside(self):
        ...
        assert abs(assoc.node_geometries["Z"].beam_azimuth_deg - _broadside(_RX_A)) < 1e-4
...
class TestResolveBeamAzimuth:
    def test_explicit_azimuth_kept(self):
        az = resolve_beam_azimuth_deg({"beam_azimuth_deg": 123.0}, _RX_A[0], _RX_A[1], _TX_LAT, _TX_LON)
        assert az == 123.0
    ...
    def test_unparseable_falls_back_to_broadside(self):
        az = resolve_beam_azimuth_deg({"beam_azimuth_deg": "north"}, _RX_A[0], _RX_A[1], _TX_LAT, _TX_LON)
        assert abs(az - _broadside(_RX_A)) < 1e-4

    def test_nan_falls_back_to_broadside(self):
        az = resolve_beam_azimuth_deg({"beam_azimuth_deg": float("nan")}, _RX_A[0], _RX_A[1], _TX_LAT, _TX_LON)
        assert math.isfinite(az)
        assert abs(az - _broadside(_RX_A)) < 1e-4

    def test_inf_falls_back_to_broadside(self):
        az = resolve_beam_azimuth_deg({"beam_azimuth_deg": float("inf")}, _RX_A[0], _RX_A[1], _TX_LAT, _TX_LON)
        assert math.isfinite(az)
        assert abs(az - _broadside(_RX_A)) < 1e-4
