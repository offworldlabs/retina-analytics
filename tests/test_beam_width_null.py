...
_resolve_fov_prior's no-TX branch returns (None, None) of its own accord, which
is a different thing and stays: see TestFovPriorOmniBranchUnchanged.
"""
...
class TestFovPriorNullWidth:
    """The learned-FOV prior takes the same node-supplied width.

    _in_theoretical_wedge already coerced a None width to the nominal at the
    point of use, so this is not a second crash — but the prior was persisted
    and reported as null while behaving as 42°, and resolving at registration
    makes the stored value the one actually in force.
    """

    def test_aimed_node_null_width_prior_is_nominal(self):
        m = NodeAnalyticsManager()
        m.register_node("N", {**_MGR_CFG, "beam_azimuth_deg": 123.0,
                              "beam_width_deg": None})
        ec = m.empirical_coverages["N"]
        assert ec.prior_azimuth_deg == 123.0
        assert ec.prior_width_deg == YAGI_BEAM_WIDTH_DEG

    def test_unaimed_node_with_tx_null_width_prior_is_nominal(self):
        m = NodeAnalyticsManager()
        m.register_node("N", {**_MGR_CFG, "beam_width_deg": None})
        ec = m.empirical_coverages["N"]
        assert ec.prior_azimuth_deg is not None   # broadside off the RX→TX baseline
        assert ec.prior_width_deg == YAGI_BEAM_WIDTH_DEG


class TestFovPriorOmniBranchUnchanged:
    """A node with neither an aim nor a TX keeps its omni (None, None) prior.

    That None width is internal, not the node's: there is no wedge for it to
    describe, and _in_theoretical_wedge reads the pair together so a None
    azimuth already admits every bin. Resolving it to the nominal would relabel
    an omni prior as a 42° one wherever the pair is serialised, so the branch is
    deliberately left out of the fix.
    """

    _NO_TX = dict(rx_lat=_MGR_RX_LAT, rx_lon=_MGR_RX_LON, max_range_km=50)

    def test_no_tx_no_aim_prior_stays_omni(self):
        m = NodeAnalyticsManager()
        m.register_node("N", dict(self._NO_TX))
        ec = m.empirical_coverages["N"]
        assert ec.prior_azimuth_deg is None
        assert ec.prior_width_deg is None
        # Omni: every bin sits inside the theoretical wedge, and the None width
        # is never divided.
        assert ec._in_theoretical_wedge(0)
        assert ec._in_theoretical_wedge(35)

    def test_no_tx_null_width_prior_stays_omni(self):
        m = NodeAnalyticsManager()
        m.register_node("N", {**self._NO_TX, "beam_width_deg": None})
        ec = m.empirical_coverages["N"]
        assert ec.prior_azimuth_deg is None
        assert ec.prior_width_deg is None
        # The detection area still resolves the node's null to the nominal;
        # only the omni prior keeps its None.
        assert m.detection_areas["N"].beam_width_deg == YAGI_BEAM_WIDTH_DEG
