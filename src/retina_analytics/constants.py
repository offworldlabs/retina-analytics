...
def resolve_beam_width_deg(config):
    ...
    fires, and the None reaches the half-width divisions in ``point_in_beam``
    here and ``_point_in_beam`` in association.py as a TypeError.  A null
    resolves to YAGI_BEAM_WIDTH_DEG because every node in the fleet carries the
    same Yagi: the width is a documented hardware spec that retina-gui simply
    never asks the owner for, not a value invented to paper over the gap.
